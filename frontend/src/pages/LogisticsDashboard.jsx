...
import {
    ...
    ShieldCheck, Sprout, ShoppingBag, Sparkles, Route as RouteIcon
} from 'lucide-react';
    ...
    return (
        <div className="min-h-screen bg-[#F4F8F4] text-[#0F172A] font-sans selection:bg-[#10B981] selection:text-[#022C22]">
            ...
            <header className="sticky top-0 z-40 bg-[#062319] text-white border-b border-[#10B981]/25 shadow-lg">
                <div className="px-4 sm:px-8 py-3.5 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <Link
                            to="/farmer"
                            className="hidden lg:inline-flex text-xs font-semibold text-gray-300 hover:text-white px-3 py-1.5 rounded-xl hover:bg-white/10 transition items-center gap-1.5"
                        >
                        ...
                        <span className="hidden sm:flex bg-[#10B981]/20 text-[#34D399] text-xs px-3 py-1.5 rounded-full font-mono border border-[#10B981]/30 items-center gap-1.5 font-bold">
                        ...
                        <div className="hidden md:block text-right">
                            ...
                            <div className="text-[10px] text-gray-400 font-mono">{profile.vehicle_type || 'Fleet Partner'}</div>
                        </div>
                    </div>
                </div>

                {/* Sub-Navigation Tabs */}
                <div className="px-4 sm:px-8 flex items-center gap-2 overflow-x-auto no-scrollbar border-t border-white/10 py-2 bg-[#041d15]">
                    {menuItems.map(item => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {item.badge !== undefined && (
                                    <span className="text-[10px] px-1.5 py-0.5 rounded-md font-mono bg-white text-[#047857] font-bold">
                                        {item.badge}
                                    </span>
                                )}
                            </button>
            ...
            <main className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto space-y-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                            ...
                            <div className="bg-white border border-gray-200 p-5 rounded-2xl shadow-sm">
                                ...
                                <div className="text-3xl font-bold text-[#0F172A] font-mono mt-1">1.6 Tons</div>
                                <div className="text-[11px] text-gray-500 mt-1 font-medium">Multi-Farm Aggregation</div>
                            </div>
                            <div className="bg-white border border-gray-200 p-5 rounded-2xl shadow-sm">
                                <span className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Est. Earnings</span>
                                <div className="text-3xl font-bold text-[#0F172A] font-mono mt-1">₹2,850</div>
                                <div className="text-[11px] text-emerald-700 mt-1 font-bold">Instant Escrow Release</div>
                            </div>
                            <div className="bg-white border border-gray-200 p-5 rounded-2xl shadow-sm">
                                <span className="text-xs text-gray-500 font-semibold uppercase tracking-wider">Driver Rating</span>
                                <div className="text-3xl font-bold text-[#0F172A] font-mono mt-1 flex items-center gap-2">
                                    5.0 <Star size={20} className="text-amber-500 fill-amber-500" />
                                </div>
                                <div className="text-[11px] text-gray-500 mt-1 font-medium">{profile.trips_completed || 42} Trips Completed</div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                                <h3 className="text-lg font-bold mb-1">Fleet & Driver Profile</h3>
                                <p className="text-xs text-gray-500">
                                    Operating Region: <span className="font-semibold text-gray-900">{profile.operating_region || 'Pune Rural'}</span> • Rate: <span className="font-semibold text-gray-900">₹{profile.rate_per_km || '25'}/km</span>
                                </p>
                            </div>
                            <button
                                onClick={() => setShowVehicleModal(true)}
                                className="px-5 py-3 bg-[#059669] hover:bg-[#047857] text-white rounded-xl font-bold text-sm transition flex items-center gap-2 shadow"
                            >
                                <Truck size={16} /> View Vehicle Details ({vehicles.length})
                            </button>
                        </div>

                        {/* Core Route Optimizer Engine Widget */}
                        ...
                        {/* Real-Time Farm-Gate Verification & Pickups Manifest */}
                ...
                {/* --- 2. TASKS & ESCROW PICKUPS --- */}
                {activeView === 'tasks' && (
                    <div className="space-y-6">
                        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
                            <h2 className="text-2xl font-bold text-gray-900 font-serif">Pickup & Drop Tasks</h2>
                            <p className="text-xs text-gray-500">Optimized sequence for maximum fuel efficiency and minimal transit loss</p>
                        </div>

                        {/* Waypoints Manifest inside Tasks View */}
                            ...
                            <div key={task.id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-200">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs mb-4">
                                    ...
                                    <div className="flex items-start gap-2 bg-amber-50 p-3 rounded-xl">
                                        <Navigation size={16} className="text-amber-600 mt-0.5" />
                                        <div><span className="block text-amber-600 font-bold">DROP</span> {task.drop} <br /> Est. Time: {task.time}</div>
                                    </div>
                                </div>
                                <button
                                    onClick={() => showToast('✅ Route navigation launched for stop ' + (idx + 1))}
                                    className="w-full py-3 bg-gradient-to-r from-[#059669] to-[#047857] text-white rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow"
                                >
                                    <Navigation size={16} /> Start Live Navigation ({task.distance})
                                </button>
                            </div>
                        ))}
                    </div>
                ...
                {activeView === 'vehicle' && (
                    <div className="max-w-3xl mx-auto bg-white border border-gray-200 rounded-3xl p-8 space-y-6 shadow-sm">
                        <div className="flex items-center gap-4 pb-6 border-b border-gray-100">
                            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#059669] to-[#047857] text-white flex items-center justify-center font-bold text-2xl shadow">
                                {name[0] || 'L'}
                            </div>
                            <div>
                                <h2 className="text-2xl font-bold text-gray-900">{name}</h2>
                                <p className="text-xs text-emerald-700 font-mono font-bold flex items-center gap-1">
                                    <CheckCircle2 size={14} /> {profile.account_category ? profile.account_category.replace('_', ' ') : 'Verified Partner'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200">
                            <h3 className="text-sm font-bold mb-3 flex items-center gap-2 text-gray-900">
                                <FileText size={16} className="text-emerald-700" /> Driving License
                            </h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-mono">
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <span className="text-gray-500 block mb-1">Operating Region</span>
                                <span className="text-gray-900 font-bold">{profile.operating_region || 'N/A'}</span>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <span className="text-gray-500 block mb-1">Service Radius</span>
                                <span className="text-gray-900 font-bold">{profile.service_radius || 'N/A'}</span>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <span className="text-gray-500 block mb-1">Availability</span>
                                <span className="text-gray-900 font-bold">{profile.availability_schedule || 'N/A'}</span>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <span className="text-gray-500 block mb-1">Rate per km</span>
                                <span className="text-gray-900 font-bold">₹{profile.rate_per_km || 'N/A'}</span>
                            </div>
                        </div>

                        <button
                            onClick={() => setShowVehicleModal(true)}
                            className="w-full mt-4 py-3 bg-gray-900 hover:bg-black text-white rounded-xl font-bold text-sm transition flex items-center justify-center gap-2 shadow"
                        >
                ...
                {activeView === 'performance' && (
                    <div className="space-y-6">
                        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
                            <h2 className="text-2xl font-bold text-gray-900 font-serif">Performance & Ratings</h2>
                            <p className="text-xs text-gray-500">Based on feedback from farmers, FPOs, and wholesale buyers</p>
                        </div>
                        <div className="grid md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white border border-gray-200 p-6 rounded-3xl shadow-sm text-center">
                                <Package size={32} className="mx-auto text-emerald-700 mb-2" />
                            ...
                            <div className="bg-white border border-gray-200 p-6 rounded-3xl shadow-sm text-center">
                                ...
                                <p className="text-xs text-gray-500 mt-1">Transit Damage Incidents</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {activeView === 'notifications' && (
                    <div className="space-y-6">
                        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
                            <h2 className="text-2xl font-bold text-gray-900 font-serif">Alerts & Notifications</h2>
                        </div>
                        <div className="space-y-4">
                            <div className="bg-white border border-red-200 p-5 rounded-2xl shadow-sm flex items-start gap-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-red-700">Urgent Perishable Pickup</h4>
                                    <p className="text-xs text-gray-500 mt-1">500kg Tomatoes from Ramesh Patel need immediate dispatch before 11:30 AM.</p>
                                </div>
                            </div>
                            <div className="bg-white border border-blue-200 p-5 rounded-2xl shadow-sm flex items-start gap-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-blue-700">New Task Assigned</h4>
                                    <p className="text-xs text-gray-500 mt-1">Route #AG-1234 assigned. Multi-farm aggregation waypoint sequence ready.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-3xl p-6 max-w-2xl w-full space-y-5 shadow-2xl max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center sticky top-0 bg-white pb-3 border-b border-gray-100">
                            <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                                <Truck size={20} className="text-emerald-700" /> Registered Vehicles ({vehicles.length})
                            </h3>
                            <button onClick={() => setShowVehicleModal(false)} className="text-gray-400 hover:text-gray-700">
                                <X size={24} />
                            </button>
                        </div>
                            ...
                            {vehicles.map((v, index) => (
                                <div key={index} className="p-5 bg-gray-50 rounded-2xl border border-gray-200">
                                    <h4 className="font-bold text-lg text-gray-900 mb-3">
                                        {v.vehicle_type ? v.vehicle_type.replace('-', ' ').toUpperCase() : 'VEHICLE'} - {v.vehicle_reg_number}
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            {v.front_photo_url ? (
                                                <img src={v.front_photo_url} alt="Front" className="rounded-xl w-full h-32 object-cover border border-gray-200" />
                                            ) : (
                                                <div className="w-full h-32 bg-gray-200 rounded-xl flex items-center justify-center text-gray-400 text-xs">No photo</div>
                                            )}
                                        </div>
                                        <div>
                                            <p className="text-xs font-bold text-gray-500 mb-1 flex items-center gap-1">
                                            ...
                                            {v.back_photo_url ? (
                                                <img src={v.back_photo_url} alt="Back" className="rounded-xl w-full h-32 object-cover border border-gray-200" />
                                            ) : (
                                                <div className="w-full h-32 bg-gray-200 rounded-xl flex items-center justify-center text-gray-400 text-xs">No photo</div>
                                            )}
                                        </div>
                                    ...
                                    <div className="mt-3 text-xs font-mono">
                                        ...
                                        <span className="font-bold text-gray-900">{v.load_capacity || 'N/A'}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
